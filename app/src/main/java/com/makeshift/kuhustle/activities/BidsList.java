...
import com.makeshift.kuhustle.adapters.BidsRecyclerViewAdapter;
...
public class BidsList extends AppCompatActivity {
    ...
    private void setUp() {
        ...
        final ArrayList<BidListItem> bids = new ArrayList<>();

        for (int i = 0; i < 100; i++) {
            bids.add(new BidListItem("Arya Stark", "You know nothing lorem ipsum dolor sit amet winter is coming", "Ksh.50,000", "8 mins", 7, R.drawable.profile_picture));
        }
        ...
        mAdapter = new BidsRecyclerViewAdapter(bids);
        mRecyclerView.setAdapter(new SlideInBottomAnimationAdapter(mAdapter));
        mRecyclerView.addOnItemTouchListener(
                new RecyclerItemClickListener(getApplicationContext(), new RecyclerItemClickListener.OnItemClickListener() {
                    @Override
                    public void onItemClick(View view, int position) {
                        Toast.makeText(getApplicationContext(), bids.get(position).getUsername() + " " + bids.get(position).getMessage(), Toast.LENGTH_SHORT).show();
