...
public class NotificationsList extends AppCompatActivity {
    ...
    private void setUp() {
        ...
        final ArrayList<NotificationListItem> notifications = new ArrayList<>();

        for (int i = 0; i < 100; i++) {
            notifications.add(new NotificationListItem("New bid!", "You know nothing lorem ipsum dolor sit amet winter is coming", "5 mins"));
        }
        ...
        mAdapter = new NotificationsRecyclerViewAdapter(notifications);
        mRecyclerView.setAdapter(new SlideInBottomAnimationAdapter(mAdapter));
        mRecyclerView.addOnItemTouchListener(
                new RecyclerItemClickListener(getApplicationContext(), new RecyclerItemClickListener.OnItemClickListener() {
                    @Override
                    public void onItemClick(View view, int position) {
                        Toast.makeText(getApplicationContext(), notifications.get(position).getTitle() + " " + notifications.get(position).getMessage(), Toast.LENGTH_SHORT).show();
